...
import { AlertTriangle, FileDigit, CalendarDays, Shield } from "lucide-react";
  ...
  const fetchApplicationDetails = async (id: string) => {
    ...
    try {
      ...
      if (result.success && result.application) {
        ...
        setIsGenuine(result.application.warishApplicationStatus === "approved");
        setIsOpen(true);
      } else {
            ...
            <Alert variant="destructive" className="mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <AlertDescription>
                    This certificate has not been approved by the authorities. The information presented may be incomplete or inaccurate.
                  </AlertDescription>
                </div>
              </div>
            </Alert>
            ...
            <CardContent className="space-y-6 py-6">
              <div className="grid grid-cols-2 gap-6 mb-8 bg-indigo-50 p-4 rounded-lg border border-indigo-200">
                ...
                <div className="space-y-2 text-right">
                  <div className="flex items-center gap-2 justify-end">
                    <div>
                      <p className="text-sm font-semibold text-gray-600">Date of Issue</p>
                      <p className="text-gray-900 font-medium">
                        {application.warishRefDate ? formatDate(application.warishRefDate) : "NA"}
                      </p>
                    </div>
              ...
              <div className="space-y-4 text-gray-700">
                <p className="leading-relaxed font-serif">
                  Certified that late <span className="font-semibold">{application.nameOfDeceased}</span>,{" "}
                  ...
                  residing at <span className="font-semibold">{application.villageName}</span> Village,{" "}
                  <span className="font-semibold">{application.postOffice}</span> Post Office, Hili Police Station of Dakshin Dinajpur District, West Bengal State, expired on{" "}
                  <span className="font-semibold">
                    {application.dateOfDeath ? formatDate(application.dateOfDeath) : ""}
                  </span>, leaving behind the following persons as his/her legal heirs:
                </p>
              ...
              <div className="mt-8 pt-4 border-t-2 border-indigo-100">
                <p className="text-sm text-gray-500 text-center">
                  This certificate is issued based on information provided by the applicant and verification by local authorities
                </p>
              </div>
            </CardContent>
          </Card>
