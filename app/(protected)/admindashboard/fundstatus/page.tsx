...
const Fundstatus = async ({
  ...
  const effectiveYear = searchParams.year ?? currentFY;
  ...
  if (searchParams.nit) {
    try {
      parsedNit = Number.parseInt(searchParams.nit);
  ...
  const works = await db.worksDetail.findMany({
    where: {
      ...
      ...(searchParams.fundType && {
        ApprovedActionPlanDetails: { schemeName: searchParams.fundType },
      }),
      tenderStatus: { in: ["AOC"] },
    },
    include: {
    ...
    <div className="p-4 space-y-6">
      <Suspense fallback={<FilterSkeleton />}>
        <CombinedFilter
          ...
          fundTypes={fundTypes}
          selectedNit={searchParams.nit}
          selectedFundType={searchParams.fundType}
      ...
      <WorksTabs works={processedWorks} />
