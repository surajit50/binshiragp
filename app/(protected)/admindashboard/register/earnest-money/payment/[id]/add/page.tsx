...
export default function AddPaymentPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchEmd = async () => {
      try {
        const response = await fetch(`/api/earnest-money/${params.id}`);
  ...
  const onSubmit = async (values: FormValues) => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/earnest-money/${params.id}/payment`, {
        method: "POST",
        headers: {
      ...
      router.push(
        `/admindashboard/register/earnest-money/payment/${params.id}`
      );
    } catch (error) {
    ...
    <div className="container mx-auto p-4">
      <div className="flex items-center space-x-4 mb-6">
        <Button variant="ghost" size="icon" asChild>
          <Link
            href={`/admindashboard/register/earnest-money/payment/${params.id}`}
          >
