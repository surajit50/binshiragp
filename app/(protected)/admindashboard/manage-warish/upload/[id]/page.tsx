...
import { ChevronLeft, FileText, FileCheck, FileX2, UploadCloud, ExternalLink } from "lucide-react"
...
export default function WarishDocumentUpload({ params }: { params: { id: string } }) {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/warish/documents/${params.id}`)
    ...
    <div className="container max-w-5xl py-10 px-4 md:px-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Upload and manage required documents</p>
          </div>
        </div>
          ...
          return (
            ...
            >
              <CardHeader className="pb-3 flex flex-row items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <div className={`p-1.5 rounded-md ${isUploaded ? "bg-green-100" : "bg-muted"}`}>
                      {isUploaded ? (
                    ...
                    <CardTitle className="text-base font-semibold">{docType.title}</CardTitle>
                  </div>
              ...
              <CardFooter className="flex justify-between items-center pt-0">
                ...
                >
                  <Button variant={isUploaded ? "secondary" : "default"} size="sm" className="w-full">
                    {isUploaded ? (
                      <>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription>View all documents uploaded for this application</CardDescription>
        </CardHeader>
        <Separator />
        <CardContent className="pt-6">
          {loading ? (
                  ...
                  {documents.map((document) => (
                    <TableRow key={document.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" onClick={() => window.open(document.cloudinaryUrl, "_blank")}>
                          <ExternalLink className="h-4 w-4 mr-2" />
                          View
                        </Button>
                      </TableCell>
            ...
            <Alert variant="default" className="bg-muted/50">
              ...
              <AlertDescription>Upload required documents using the cards above</AlertDescription>
            </Alert>
          )}
        </CardContent>
      </Card>
