...
export async function createNestedWarishDetails(
  ...
  const duplicateApplication = await db.warishApplication.findFirst({
    where: {
      nameOfDeceased: {
        equals: data.nameOfDeceased,
  ...
  console.log("Duplicate check:", { 
    searchedName: data.nameOfDeceased, 
    searchedDate: data.dateOfDeath,
    found: !!duplicateApplication 
  });
  
  if (duplicateApplication) {
    console.log("Duplicate found:", duplicateApplication);
    return {
      success: false,
      message: "Duplicate application found. Please contact Gram Panchayat office for more information.",
      errors: {
        nameOfDeceased: ["A record with this name and date already exists"],
        dateOfDeath: ["A record with this name and date already exists"]
      }
    };
  }
  try {
    ...
    revalidatePath("/admindashboard/warishapplication");
...
export async function searchWarishApplications(searchParams: {
  ...
  const { deceasedName, acknowledgementNo, applicantName } = searchParams;
