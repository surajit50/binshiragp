...
export const addPaymentDetails = async (
  ...
  try {
    ...
    if (existingWork.paymentDetails.length > 0) {
      const existingBill = existingWork.paymentDetails.find(
        (bill) => bill.billType === "Final Bill"
      );
      if (existingBill) {
        return { error: "allready bill added" };
      }
    }
