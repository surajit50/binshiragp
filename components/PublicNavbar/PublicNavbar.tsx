    ...
    submenu: [
      {
        ...
        submenu: [
          { name: "Online Applications", href: "/services/e-governance/applications" },
          { name: "Document Verification", href: "/services/e-governance/verification" },
          { name: "Grievance Redressal", href: "/services/e-governance/grievance" },
        ],
      },
      {
        name: "Social Welfare",
        ...
        submenu: [
          ...
          { name: "Education Support", href: "/services/social-welfare/education" },
          { name: "Healthcare Initiatives", href: "/services/social-welfare/healthcare" },
        ],
      },
    ...
    <div className="relative group" ref={dropdownRef}>
      <motion.button
        ...
        className={cn(
          ...
          isActive ? "text-primary bg-primary/10" : "text-foreground/80 hover:bg-accent hover:text-foreground"
        )}
      ...
      <AnimatePresence>
        {isOpen && item.submenu && (
          <motion.div
            ...
            className="absolute left-0 mt-2 w-56 rounded-xl shadow-lg bg-background border"
          >
                ...
                <div key={subItem.name}>
                  {subItem.submenu ? (
                    ...
                    <motion.div whileHover={{ scale: 1.02 }}>
                      <Link
                        href={subItem.href}
                        className="flex items-center px-4 py-2.5 text-sm rounded-md hover:bg-accent transition-colors"
                      >
                        {subItem.name}
                      </Link>
                    </motion.div>
...
function NestedDropdown({ item }: { item: NavItem }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (nestedDropdownRef.current && !nestedDropdownRef.current.contains(event.target as Node)) {
        setIsOpen(false);
      }
    };
    document.addEventListener("mousedown", handleClickOutside);
  ...
  return (
    ...
    >
      <motion.button
        className="flex items-center justify-between w-full px-4 py-2.5 text-sm rounded-md hover:bg-accent transition-colors"
        role="menuitem"
        whileHover={{ x: 2 }}
      >
        <span>{item.name}</span>
      ...
      <AnimatePresence>
        {isOpen && item.submenu && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -10 }}
            transition={{ duration: 0.15 }}
            className="absolute left-full top-0 -ml-1 w-56 rounded-xl shadow-lg bg-background border"
          >
            <div className="p-2 space-y-1">
              {item.submenu.map((subItem) => (
                <motion.div key={subItem.name} whileHover={{ scale: 1.02 }}>
                  <Link
                    href={subItem.href}
                    className="block px-4 py-2.5 text-sm rounded-md hover:bg-accent transition-colors"
                  >
                    {subItem.name}
                  </Link>
                </motion.div>
...
function MobileMenuItem({ item }: { item: NavItem }) {
  ...
  return (
    <div className="border-b last:border-0 border-border/20">
      {item.submenu ? (
        <>
          <motion.button
            onClick={() => setIsOpen(!isOpen)}
            className={cn(
              "flex items-center justify-between w-full px-4 py-3 text-base font-medium",
              pathname.startsWith(item.href) ? "text-primary" : "text-foreground/80"
            )}
            whileTap={{ scale: 0.98 }}
          >
            <span>{item.name}</span>
            <ChevronRight className={cn("h-5 w-5 transform transition-transform", isOpen && "rotate-90")} />
          </motion.button>
          <AnimatePresence>
            {isOpen && (
              <motion.div
...
export function PublicNavbar({ className, ...props }: PublicNavbarProps) {
  ...
  return (
    <nav className={cn("relative z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80", className)} {...props}>
      {/* Desktop Navigation */}
      <div className="hidden lg:flex items-center gap-1">
        {navItems.map((item) => (
          <React.Fragment key={item.name}>
            {item.submenu ? (
              <DropdownMenu item={item} isActive={pathname.startsWith(item.href)} />
            ) : (
              <motion.div whileHover={{ scale: 1.02 }}>
                <Link
                  href={item.href}
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="outline"
              size="icon"
              className="bg-background/80 backdrop-blur-sm hover:bg-accent/50"
              aria-label="Open menu"
            >
              <Menu className="h-5 w-5" />
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-[85vw] max-w-sm sm:w-[300px]">
